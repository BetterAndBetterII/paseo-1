...
function latestHistoryPrompt(agentIndex: number): string {
  return `desktop-version-h176-a${agentIndex + 1}-turn-88: emit 32 coalesced agent stream updates`;
}

function oldestHistoryPrompt(agentIndex: number): string {
  return `desktop-version-h176-a${agentIndex + 1}-turn-1: emit 32 coalesced agent stream updates`;
}
...
async function clickWorkspaceTab(page: Page, agentId: string): Promise<void> {
  const tab = page.getByTestId(`workspace-tab-agent_${agentId}`);
  await tab.scrollIntoViewIfNeeded();
  const bounds = await tab.boundingBox();
  if (!bounds) throw new Error(`Tab ${agentId} has no bounds`);
...
async function openHistoryTabs(
  ...
  for (let index = 0; index < agents.length; index += 1) {
    ...
    await waitForVisibleTimelineText(page, latestHistoryPrompt(index));
    if (index === 0) {
  ...
  for (let index = 0; index < agents.length; index += 1) {
    const agent = agents[index];
    await clickWorkspaceTab(page, agent.id);
    await waitForVisibleTimelineText(page, latestHistoryPrompt(index));
    await loadOldestHistory(page, oldestHistoryPrompt(index));
...
async function enableRenderProfiling(
  page: Page,
  firstAgent: FixtureAgent,
  lastAgent: FixtureAgent,
): Promise<void> {
  await page.evaluate(() => {
  ...
  await clickWorkspaceTab(page, firstAgent.id);
  await waitForVisibleTimelineText(page, latestHistoryPrompt(0));
  await page.waitForFunction(() => {
    return typeof (window as BenchmarkWindow).__PASEO_RESET_RENDER_PROFILE__ === "function";
  });
  await clickWorkspaceTab(page, lastAgent.id);
  await waitForVisibleTimelineText(page, latestHistoryPrompt(7));
}
...
async function armSwitchMeasurement(
  ...
  await page.evaluate(
    ({ targetAgentId, targetTitle, targetPrompt, previousPrompt, timeoutMs }) => {
      const state = window as BenchmarkWindow;
      const target = document.querySelector<HTMLElement>(
        `[data-testid="workspace-tab-agent_${targetAgentId}"]`,
      );
...
async function measureSwitch(
  ...
  await clickWorkspaceTab(page, target.id);
...
async function createAnimatedGif(input: {
  ...
  await input.page.getByTestId(`workspace-tab-agent_${input.agents[0]!.id}`).waitFor({
    timeout: 60_000,
  });
  for (const agent of input.agents) {
    await clickWorkspaceTab(input.page, agent.id);
...
async function main(): Promise<void> {
  ...
  const workspace = manifest.workspaces.find((entry) => entry.targetHistoryItems === 176);
  if (!workspace) throw new Error("Fixture has no H176 workspace");
  const historyAgents = workspace.agents.filter((agent) => agent.role === "history");
  const markdownAgent = manifest.workspaces
    .flatMap((entry) => entry.agents)
    .find((agent) => agent.role === "markdown-cold");
  const nonHistoryAgents = workspace.agents.filter((agent) => agent.role !== "history");
  if (historyAgents.length !== 8 || !markdownAgent) {
    throw new Error("Fixture must contain 8 H176 history agents and one cold Markdown agent");
  }
  ...
  await enableRenderProfiling(page, historyAgents[0]!, historyAgents[historyAgents.length - 1]!);
  ...
  let previousPrompt = latestHistoryPrompt(historyAgents.length - 1);
  for (let cycle = 0; cycle < SWITCH_CYCLES; cycle += 1) {
    for (let index = 0; index < historyAgents.length; index += 1) {
      const agent = historyAgents[index];
      const targetPrompt = latestHistoryPrompt(index);
      switchSamples.push(await measureSwitch(page, agent, targetPrompt, previousPrompt));
  ...
  const benchmark = {
    ...
    benchmark: "desktop_version_fixture@v1",
    variant,
    ...
    fixture: {
      ...
      historyAgentIds: historyAgents.map((agent) => agent.id),
      markdownAgentId: markdownAgent.id,
    },
    workload: {
      historyItemsPerAgent: 176,
      historyAgents: historyAgents.length,
